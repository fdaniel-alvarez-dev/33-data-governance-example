...
def demo_mode() -> None:
    ...
    report_path = ARTIFACTS_DIR / "multicloud_guardrails.json"
    guard = run([sys.executable, "tools/multicloud_guardrails.py", "--format", "json", "--out", str(report_path)])
    if guard.returncode != 0:
        fail("Guardrails failed (demo mode must be offline).", output=guard.stdout)
